  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <div className="w-full h-full bg-black border border-white/10 rounded-lg overflow-hidden hover:border-white/20 transition-all duration-300 shadow-xl">
            ...
            <div className="relative h-44 overflow-hidden">
              <div className={`absolute inset-0  ${project.gradient} opacity-60`} />
              <motion.img
                src={project.image}
                alt={project.title}
              ...
              <motion.div
                ...
                className="absolute top-3 right-3 bg-white/90 backdrop-blur-sm px-2.5 py-1 rounded-full text-[10px] font-medium text-black"
              >
                Hover to flip
              </motion.div>
            </div>
            ...
            <div className="p-5">
              ...
              {/* Project Description */}
              <motion.p 
                className="text-white/50 text-xs mb-3 font-light leading-relaxed line-clamp-2"
                initial={{ x: -20, opacity: 0 }}
                animate={{ x: 0, opacity: 1 }}
                transition={{ delay: 0.2 }}
              >
              ...
              >
                {project.tech.slice(0, 3).map((tech, index) => (
                  <span
                    key={index}
                    className="px-2.5 py-0.5 text-[10px] font-light bg-white/5 text-white/60 border border-white/10 rounded-full"
                  >
                    {tech}
                  </span>
                ))}
                {project.tech.length > 3 && (
                  <span className="px-2.5 py-0.5 text-[10px] font-light bg-white/5 text-white/60 border border-white/10 rounded-full">
                    +{project.tech.length - 3}
                  </span>
                )}
              </motion.div>
        ...
        >
          <div className={`w-full h-full bg-gray-500 ${project.gradient} rounded-lg overflow-hidden shadow-2xl p-5 flex flex-col justify-between`}>
            {/* Back Content */}
            <div>
              <motion.h3 
              ...
              <div className="flex flex-wrap gap-1.5 mb-4">
                {project.tech.map((tech, index) => (
                  <motion.span
                    ...
                    className="px-2.5 py-1 text-[10px] font-medium bg-white/20 text-black backdrop-blur-sm rounded-full border border-white/30"
                  >
                    {tech}
                  </motion.span>
                ))}
            ...
            <div className="flex gap-2">
              <motion.a
                ...
                whileTap={{ scale: 0.95 }}
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs bg-white/90 hover:bg-white text-black rounded-lg transition-all duration-300 font-semibold shadow-lg"
              >
              ...
              <motion.a
                ...
                whileHover={{ scale: 1.05, y: -2 }}
                whileTap={{ scale: 0.95 }}
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 text-xs bg-black/60 hover:bg-black/80 text-white rounded-lg transition-all duration-300 font-semibold shadow-lg backdrop-blur-sm border border-white/20"
              >
